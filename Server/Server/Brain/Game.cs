    ...
    public class Game
    {
        ...
        private void PlayerBidAccepted(object sender, RecieveBidEventArgs e, int playerIndex)
        {
            ...
            if (playerIndex == currentAwaitingIndex)
            {
                if (e.Bid.HasValue)
                {
                    if (e.Bid.Value.Amount >= 5 + RoundStatus[0].TurnNumber)
                    {
        ...
        private void FinishPlayRound()
        {
            ...
            UpdateStatusToPlayers(false);
            tm = new Timer(new TimerCallback(l => StartPlayRound()), null, Viewer != null ? 4000 : 500, Viewer != null ? 4000 : 500);
        }
